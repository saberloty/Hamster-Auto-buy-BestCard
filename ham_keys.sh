...
TELEGRAM_CHANNEL_ID="@Your_Chennel" #telegram channel id
...
login() {
    ...
    response=$(curl -s $proxy_option -X POST -H "Content-Type: application/json" \
...
emulate_progress() {
    ...
    response=$(curl -s $proxy_option -X POST -H "Authorization: Bearer $client_token" \
